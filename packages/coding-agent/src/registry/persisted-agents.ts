...
// MM_SINGLE_WRITER_PER_KEY: per-key serialization for registry/session writes.
// Serializes operations per key without coupling unrelated keys.
// A failed predecessor does not poison later operations for the same key.
class KeyedOperationLane<Key> {
...
const keyedLane = new KeyedOperationLane<string>();
